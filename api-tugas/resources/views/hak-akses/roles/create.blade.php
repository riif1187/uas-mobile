...
<div class="container-fluid">
    ...
    <div class="card shadow-sm">
        <div class="card-header text-white d-flex align-items-center gap-2"
            ...
            <strong>Form Tambah Role</strong>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('roles.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="nama_role" class="form-select @error('nama_role') is-invalid @enderror">
                        ...
                        <optgroup label="Tingkat Dasar">
                            <option value="mahasiswa"   {{ old('nama_role') == 'mahasiswa'   ? 'selected' : '' }}>Mahasiswa</option>
                            <option value="member"      {{ old('nama_role') == 'member'      ? 'selected' : '' }}>Member</option>
                        </optgroup>
                        <optgroup label="Tingkat Menengah">
                            <option value="operator"    {{ old('nama_role') == 'operator'    ? 'selected' : '' }}>Operator</option>
                            <option value="editor"      {{ old('nama_role') == 'editor'      ? 'selected' : '' }}>Editor</option>
                            <option value="dosen"       {{ old('nama_role') == 'dosen'       ? 'selected' : '' }}>Dosen</option>
                            <option value="kaprodi"     {{ old('nama_role') == 'kaprodi'     ? 'selected' : '' }}>Kepala Program Studi</option>
                        </optgroup>
                        <optgroup label="Tingkat Tinggi">
                            <option value="supervisor"  {{ old('nama_role') == 'supervisor'  ? 'selected' : '' }}>Supervisor</option>
                            <option value="administrator" {{ old('nama_role') == 'administrator' ? 'selected' : '' }}>Administrator</option>
                            <option value="superadmin"  {{ old('nama_role') == 'superadmin'  ? 'selected' : '' }}>Super Admin</option>
                        </optgroup>
                    </select>
                    @error('nama_role')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">Pilih tingkatan role sesuai kebutuhan akses.</small>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <small class="text-muted">User yang dipilih akan langsung mendapatkan role ini setelah disimpan.</small>
                </div>
